...
/******/ 	var hotCurrentHash = "10bc1abbdf51e068961b"; // eslint-disable-line no-unused-vars
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _habituationGWR = __webpack_require__(2);

	var _habituationGWR2 = _interopRequireDefault(_habituationGWR);

	var _oneUnitWithPosFB = __webpack_require__(3);

	var _oneUnitWithPosFB2 = _interopRequireDefault(_oneUnitWithPosFB);

	function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

	(0, _habituationGWR2.default)();
	(0, _oneUnitWithPosFB2.default)();
...
/***/ function(module, exports) {
	...
	exports.default = oneUnitWithPosFB;
	// inputFlag = 1 for pulse, 2 for step
	function oneUnitWithPosFB() {
	  var inputFlag = arguments.length <= 0 || arguments[0] === undefined ? 1 : arguments[0];

	  var cut = 0; // set cut-off
	  var sat = 10; // set saturation

	  var tEnd = 100;
	  var timeSteps = tEnd + 1;

	  var input = 1;
	  var feedback = 1.05;

	  var x = new Array(timeSteps).fill(0);
	  var start = 11;
	  ...
	  var y = new Array(timeSteps).fill(0);

	  for (var i = 2; i < timeSteps; i++) {
	    y[i] = feedback * y[i - 1] + input * x[i - 1];
	    if (y[i] < cut) y[i] = cut;
	    if (y[i] > sat) y[i] = sat;
	  }

	  var inputData = x.map(function (num, index) {
	    return { x: index, y: num };
	  });
	  var outputData = y.map(function (num, index) {
	    return { x: index, y: num };
	  });
	...
	var render2 = function render2(data, selector, options) {
	  var vis = d3.select(selector),
	      WIDTH = 600,
	      HEIGHT = 200,
	      MARGINS = {
